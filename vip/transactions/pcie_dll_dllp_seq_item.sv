...
class pcie_dll_dllp_seq_item extends pcie_dll_base_seq_item;
  ...
  constraint initfc1_credit{
    if (dllp_type inside {DLLP_INITFC1_P, DLLP_INITFC2_P}) {
      hdr_scale  == cfg.init_fc_hdr_scale_p;
      hdr_FC     == cfg.init_fc_hdr_p;
      data_scale == cfg.init_fc_data_scale_p;
      data_FC    == cfg.init_fc_data_p;
    } 

    else if (dllp_type inside {DLLP_INITFC1_NP, DLLP_INITFC2_NP}) {
      hdr_scale  == cfg.init_fc_hdr_scale_np;
      hdr_FC     == cfg.init_fc_hdr_np;
      data_scale == cfg.init_fc_data_scale_np;
      data_FC    == cfg.init_fc_data_np;
    }

    else if (dllp_type inside {DLLP_INITFC1_CPL, DLLP_INITFC2_CPL}) {
      hdr_scale  == cfg.init_fc_hdr_scale_cpl;
      hdr_FC     == cfg.init_fc_hdr_cpl;
      data_scale == cfg.init_fc_data_scale_cpl;
      data_FC    == cfg.init_fc_data_cpl;
  } 
}
