class test_base_drop_link extends pcie_dll_test_base;
  ...
  pcie_dlcmsm_state_e link_drop_target_state;

  `uvm_component_utils(test_base_drop_link)

  function new(string name = "test_base_drop_link", uvm_component parent = null);
  ...
  task run_phase(uvm_phase phase);
    ...
    `uvm_info("TEST", "Starting  test with random link drop for both RC and EP", UVM_LOW)

    repeat (10)
    begin

        if (!std::randomize(link_drop_target_state)) begin
            `uvm_error("RAND_FAIL", "Failed to randomize link_drop_target DLSM state")
        end
        `uvm_info("TEST", $sformatf("Waiting for State Manager to %s...", link_drop_target_state.name()), UVM_LOW)
        fork
        begin
            wait (env_rc.my_cfg.dlsm_state == link_drop_target_state);
            `uvm_info("TEST",$sformatf( "the RC reached %s!!!!!!",link_drop_target_state.name()), UVM_LOW)
            
        end
        begin
            wait(env_ep.my_cfg.dlsm_state == link_drop_target_state);
            `uvm_info("TEST",$sformatf( "the EP reached %s!!!!!!",link_drop_target_state.name()), UVM_LOW)
