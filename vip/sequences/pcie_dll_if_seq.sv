class pcie_dll_if_seq extends uvm_sequence #(pcie_dll_if_seq_item);
    ...
    task body();
        ...
        `uvm_info("RESET", $sformatf("-----Starting sequence with drop_link = %b, cycles_num = %0d at time = %0t------", seq_item.drop_link, seq_item.cycles_num, $time), UVM_LOW)
        finish_item(seq_item);
        `uvm_info("RESET", $sformatf("-----Finished sequence with drop_link = %b, cycles_num = %0d at time = %0t------", seq_item.drop_link, seq_item.cycles_num, $time), UVM_LOW)
    endtask
