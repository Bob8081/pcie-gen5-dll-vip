class pcie_dll_common_checks extends uvm_object;
  ...
  function void calling_common_checks();
    if (tx_updated && rx_updated) begin
      ...
      `uvm_info("SCOREBOARD", $sformatf("Current Tx state: %s, Previous Tx state: %s, Current Rx state: %s, Previous Rx state: %s", tx_curr_state.name(), tx_prev_state.name(), rx_curr_state.name(), rx_prev_state.name()), UVM_LOW)
      `uvm_info("SCOREBOARD", $sformatf("Current Tx DLLP: %s, Previous Tx DLLP: %s, Current Rx DLLP: %s, Previous Rx DLLP: %s", tx_curr_dllp_type.name(), tx_prev_dllp_type.name(), rx_curr_dllp_type.name(), rx_prev_dllp_type.name()), UVM_LOW)
      `uvm_info("SCOREBOARD", $sformatf("current Tx DLLP type: %s, current Rx DLLP type: %s", tx_curr_dllp_type.name(), rx_curr_dllp_type.name()), UVM_LOW)
    ...
    function void traffic_isolation_check ();

      pcie_dll_dllp_seq_item dllp_item = tx_dllp_item;  
...
// ------------------- DATA INTEGREITY CHECKS IMPLEMENTATIONS -------------------
    ...
    function void drop_packets ();
        ...
        if (current_state inside {DL_INIT_FC1, DL_INIT_FC2}) begin
            ...
            else if (   (current_dllp_type inside {DLLP_INITFC1_CPL } && prev_dllp_type inside {DLLP_INITFC1_P   })
    ...
    function void check_symmetric_active ();
    ...
    else if (tx_curr_state == DL_INACTIVE && tx_prev_state == DL_ACTIVE && rx_curr_state != DL_ACTIVE) begin
        `uvm_fatal("symmetric_active", "Violation: both RC and EP don't reach active state symmetrically!")
    end
    else if (rx_curr_state == DL_INACTIVE && rx_prev_state == DL_ACTIVE && tx_curr_state != DL_ACTIVE) begin
        `uvm_fatal("symmetric_active", "Violation: both RC and EP don't reach active state symmetrically!")
    end
...
bit                      no_capture_credits = 0; 
    ...
    case (rx_dllp_item.dllp_type)
        DLLP_INITFC1_P   , DLLP_INITFC2_P   : credit_type = FC_P;
        DLLP_INITFC1_NP  , DLLP_INITFC2_NP  : credit_type = FC_NP;
